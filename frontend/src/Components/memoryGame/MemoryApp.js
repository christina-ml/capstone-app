...
function MemoryApp() {
	...
	// check if all cards have been matched - user has won
	let checkAllMatches = cards.every((card) => {
		return card.matched === true;
	});

   // all cards have been matched - show confetti
    useEffect(() => {
        if (checkAllMatches && turns > 0) {
            setShowConfetti(true);

            // stop confetti after setTimeout
            setTimeout(() => {
                setShowConfetti(false);
            }, 7000);
        }
    }, [checkAllMatches, turns]);
	...
	const shuffleCards = () => {
		...
		let randomlyShuffledCards = sortedCardPairs.map((card) => ({
			...card,
			id: Math.random(),
		}));
	...
	// console.log("cards/turns:", cards, turns);
